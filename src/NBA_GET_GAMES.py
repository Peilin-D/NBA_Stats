...
def Get_All_Games(year):
    ...
    # get the teams
    try:
        city_name_Dict = pk.load(open(data_path + 'city_name.pickle'))
        name_pref_Tuples = pk.load(open(data_path + 'name_pref.pickle'))
    except IOError:
        Get_All_Teams()
        city_name_Dict = pk.load(open(data_path + 'city_name.pickle'))
        name_pref_Tuples = pk.load(open(data_path + 'name_pref.pickle'))
    ...
    games_df2.to_csv(data_path + 'Games_' + str(year) + '.csv', index=False)
